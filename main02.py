...
def match_puzzle_with_rotation(full_image_path, piece_image_path):
    ...
    # 将拼图块和完整拼图都转换到Lab颜色空间，以更好地利用颜色信息
    ...
    x_max = min(full_image.shape[1]-1, int(np.ceil(x_max)))
    y_min = max(0, int(np.floor(y_min)))
    y_max = min(full_image.shape[0]-1, int(np.ceil(y_max)))

    # 绘制匹配结果（以矩形框标记拼图位置）
    marked_image = full_image.copy()
    cv2.rectangle(marked_image, (x_min, y_min), (x_max, y_max), (0, 255, 0), 3)
